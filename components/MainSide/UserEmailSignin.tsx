...
//import prisma and add it to the Provider
 ...
 let Provider = {
      UserEmailPassSignin: { ...data.Provider.UserEmailPassSignin, prisma },
    };

const response = await Simplauth(Provider);`;

const CodeText6 = `const Provider = {
    UserEmailPassSignin: { useroremail, password },
  };
      ...
      <div className="flex-col justify-start items-start pl-4">
        ...
        <div className="flex flex-col gap-4 items-start justify-center">
          <p className="text-left text-md text-white-900">
            This has two parts => Autologin and Manual login. And both are necessary to be used.
          </p> 
          <p className="text-left text-md text-white-900">
          In brief, the autologin feature allows you to automatically log in to your account when you visit the website. 
          It works by sending a request to the server upon your visit, and if successful, you are automatically logged in. 
          The manual login feature allows you to log in manually if autologin fails. 
          It prompts you to enter your username or email and password when you visit the website.
          </p> 
          {/* ---------------------------------Autologin------------------------------------ */}
          ...
          <div className="flex flex-col gap-4 items-center justify-center">
            <br />
            <p>
            In the .env file of the project add the JWT_SECRET_KEY variable having a random secret string which can be used to encrypt and decrypt the JWT token.
            </p>
            <CopyableText code={CodeText1} />
           
            Extract the cookie named "simplauthsignin".
            <CopyableText code={CodeText2} />
            For autologin, use the useEffect hook to access the cookie value and initiate autologin on page load. 
            Pass the Provider in the request body exactly as shown in the example below.
            <br />
            <CopyableText code={CodeText3} />
            
            In the backend route, import the Simplauth library and add Prisma to the Provider. 
            Ensure that UserEmailPassPrisma is defined in the Prisma schemas. 
            Then pass the Provider as a parameter to the Simplauth library.
            <CopyableText code={CodeText4} />
            <CopyableText code={CodeText5} />
            <p>
            Simplauth will return {` { newjwt, status: "success" }`} if everything goes well. 
            This response can then be used to store newjwt in a cookie named simplauthsignin, effectively updating or refreshing the JWT to extend its validity for the next 7 days. 
          ...
          <div className="flex flex-col gap-4 items-center justify-center">
            <br />
            <p>
            If autologin fails due to reasons such as the simplauthsignin cookie being deleted, expired, or never created because the user hasn’t signed up, the user will need to log in manually by providing credentials. 
            The user must supply two things: email (or username) and password within the Provider. 
            Keep the Provider exactly as shown, and send a request to the backend as demonstrated below.
            </p>
            <CopyableText code={CodeText6} />
            Send the request to the same backend endpoint; everything in the backend will remain the same. If successful, the response will be {` { newjwt, status: "success" }`}. 
            The newjwt should then be stored in a cookie named simplauthsignin.
            ...
            . The user credentials — username or email and password as string 
            are not met, then the response will be{" "}
            {` { message: "Invalid credentials", status: "failed" }`}. If the
            email or username doesn't match then the response will be,
            {` { message: "Invalid email or username", status: "failed" }`}. If the
            password is incorrect then{` { message: "Invalid password", status: "failed" }`}.
            For any other error, the catch block will handle it and return{" "}
            ...
            <p>
            If you'd like to refer to the example code, check out this GitHub repository:
            </p>
            <Link
